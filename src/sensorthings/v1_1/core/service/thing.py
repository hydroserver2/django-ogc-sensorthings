...
class ThingService:
    def get_thing_collection(self, *args, **kwargs):
        ...
        thing = ThingDTO(
            iot_id=1, description="Test thing", iot_location_ids=[1,2,3], properties={"code": "A"}
        )
    ...
    def get_thing(self, *args, **kwargs) -> ThingDTO:
        ...
        thing = ThingDTO(
            iot_id=1, name="Test", description="Test thing", iot_location_ids=[1,2,3], properties={"code": "A"}
        )

        thing.apply_select_expand(select=["iot_id", "name"], expand={"locations": {"count": 12, "value": []}})
